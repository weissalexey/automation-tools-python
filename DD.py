...
def XMLMAIKER (fName,StrLine):
    with open(f'{mypathOUT}{fName}.XML', 'a') as out:
        out.write(StrLine.replace('&', '&amp;').replace("'","") +'\n')
...
def XMLmacher(AUF, OMEXFiele,fm):
    #time.sleep(0.5)
    #TEW_DAT = 0 if fm != fm_alt else 1
    fm_alt = fm
    print(AUF)
...
,Empkun.email as 'RecipientPhoneNo'
 --**********General-element*********************************************************************
,replace(convert(VARCHAR(255) , SLAuf.AufInfo), '&' , '&amp;') as 'Notes'
...
,CAsE when SLAPos.Vpe like 'VP' Then (select convert(int,sum (VPEAnz),0) from xxaSLAPos where Vpe like 'VP' and  AufIntNr in (select AufIntNr from XXASLAuf where aufnr in ({AUF}))) Else null END AS 'ExchangePallets1_4'
 ...
 ,SLAPos.QMAnz / SumPos.VPEAnz as 'Volume'
 ,SLAuf.LMAnz / SumPos.VPEAnz as 'LDM'
 ,DlSvCtrl as 'ADR_UN'
            ...
            for row in cursor:
                ...
                if str(TrackandTraceEmail)[:2]=='64' or len(TrackandTraceEmail)==0:
                    ...
                    if len(GoodsDescription) > 1 :
                        ...
                        GoodsDescription = GoodsDescription.replace('>', ' ')
                        GoodsDescription = GoodsDescription.replace('<', ' ')
                        GoodsDescription = GoodsDescription.replace('br', ' ')
                ...
                for line in file:
                    ...
                    if AUFTR[0] == 'AUFTR':
                        ...
                        if len(AUFTR[3])==8:
                            OMEXFiele = fm[:2]
                            print (OMEXFiele)
                            XMLmacher (AUFTR[3],OMEXFiele,fm)
            file.close
