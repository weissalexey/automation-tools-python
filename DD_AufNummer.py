"""
Script: DD_AufNummer.py
Description: [ADD DESCRIPTION HERE]
Usage: python DD_AufNummer.py
"""
...
def LiefNrUpdate (AUF,TrackTraceNr, OMEXFiele):
    ...
    #SQLLLLL = SQLLLLL + '\n' + f"""use winsped update XXASLAuf set DsiNr = 'T' + {OMEXFiele} where AufIntNr in (select AufIntNr from XXASLAuf where aufnr in  ('{AUF}'))"""
    print (SQLLLLL)
...
,CAsE 
when SLAuf.Rel = 'TJW' Then '28'   
when SLAuf.Rel = 'DD_Fre' Then '70'
when SLAuf.Rel = '9245' Then '64'
when SLAuf.Rel = 'JOJEDI' Then '75'
when SLAuf.Rel = 'DD_Bra' Then '80'
when SLAuf.Rel = 'DD_Nør' Then '92'
when SLAuf.Rel = 'DD_Bra' Then '92'
when SLAuf.Rel = '9246' Then '67'
when SLAuf.Rel = '9247' Then '64'
when SLAuf.Rel = 'StykGo' Then '78'
when SLAuf.Rel = 'Struer' Then '76'
when SLAuf.Rel = 'SDL' Then '56'
when SLAuf.Rel = 'JørviT' Then '88'
when SLAuf.Rel = 'Hansen' Then '66'
when SLAuf.Rel = 'ESGODS' Then '67'
--when SLAuf.Rel = '9242' Then '67'
--when SLAuf.Rel = '9241' Then '67'
Else SLAuf.Rel END AS 'OmexTnrAfh'
...
--when SLAuf.Rel = '9242' Then '67'
--when SLAuf.Rel = '9241' Then '67'
Else SLAuf.Rel END AS 'OmexTnrLev'
 --**********SenderAddress************************************************************************
...
where SLAuf.AufArt != 'D' and nve.AufIntNr in (select AufIntNr from XXASLAuf where aufnr in ({AUF})) --and tt.TrackandTraceEmail is null 
"""
    ...
    for row in cursor:
        ...
        OmexTnrLev = OmexTnrVia
        OmexTnrAfh = OmexTnrVia
        ...
        if len(Notes) > 1 :
            translation = translator.translate(Notes)
            Notes = translation.text.replace('&', '&amp;')
        ...
        if len(GoodsDescription) > 1 :
            translation = translator.translate(GoodsDescription)
            GoodsDescription = translation.text
        ...
        if len(AUF) > 0:
            ...
            if FBNR_ALT != AUF:
                ...
                print (FBNR)
                #if row['TT'] is not None:
                #    TrackTraceNr = str(row['TT']) 
                #else:
                #    TrackandTraceEmail = TrackTraceNr
                if FBNR_ALT != '':
                ...
                #XXX = LiefNrUpdate (AUF,TrackTraceNr, OMEXFiele)
                ...
                x = XMLMAIKER (DtSt + AUF + Bschaffen ,f'      <GoodsDescription>{BarCode}</GoodsDescription>') if len(GoodsDescription)>0 else XMLMAIKER (DtSt + AUF + Bschaffen ,f'      <GoodsDescription />')
...
if __name__ == "__main__":
    print ('Begin')
    if Test_work_flag == 0:
        with open(FLAG_PATCH, "w") as file:
            file.write("Flag DD_export")
            file.close 
    ...
    #                    OMEXFiele = fm[:2] 
    #                    print (OMEXFiele)
    #                    XMLmacher (AUFTR[3],OMEXFiele)
    #    file.close 
    #    shutil.copy2(mypathIN + fm, '//srv-wss/Schnittstellen/DansckDistribution/in/BCK')
    #    os.remove(mypathIN + fm)
        XMLmacher ('10103337')
        os.remove(FLAG_PATCH)
    else:
        WRITELOG('FLAG')
        print ('FLAG')
    print ('Ende')